---
title: 万字长文图解Qwen2.5-VL实现细节
url: https://zhuanlan.zhihu.com/p/1921289925552210138
author: 猛猿
author_badge: 公众号：大猿搬砖简记
created: 2025-06-25 20:04
modified: 2025-06-25 20:04
upvote_num: 1058
comment_num: 61
---
## 目录
...
1. 总览
    1. 1. 相比于qwen2-VL的改进
    2. 2. qwen2.5-VL训练的三个阶段
2. 一、推理请求数据格式
3. 二、整体架构
4. 三、process_vision_info
    1. 3.1 图像数据的预处理
    2. 3.2 视频数据的预处理
5. 四、Processor
    1. 4.1 self.image_processor: Qwen2VLImageProcessor
    2. 4.2 self.tokenzier
6. 五、VIT Encoder
    1. 5.1 使用3D Conv将原始图像patch转变为vit的输入token
    2. 5.2 2D rope
    3. 5.3 window attention
    4. 5.4 patch merge
7. 六、mrope
    1. 6.1 确定每个token的position_id
    2. 6.2 3D mrope

![](./assets/v2-fa725e5f69c2994059b74b6cf094888d_1440w.image.jpg)

大家好，这篇文章是今年年初就写的了，**为了防止盗版猖獗，一直没有公开** ，只给我的同事和周边做框架开发的朋友们内部分享过。时隔半年，qwen2.5-VL应该已经渗入到mllm开发的各个场景中了，所以我选择在这个时候放出来。
...
[【必看】历史技术文章导航](https://zhuanlan.zhihu.com/p/654910335)
...
这里以“单条数据-图像”推理为例，更多例子请参见[这里](https://github.com/QwenLM/Qwen2.5-VL)（例如视频推理，batch推理等）
...
![](./assets/v2-cffd1cbd01d21096357496010da2d4df_r.jpg)

*来自qwen论文*

以下把图像和视频数据统称为“视觉数据”

- [process_vision_info](https://github.com/QwenLM/Qwen2.5-VL/blob/c15045f8829fee29d4b3996e068775fe6a5855db/qwen-vl-utils/src/qwen_vl_utils/vision_process.py#L352):  视觉数据在进入vit前，会先做一些预处理，包括对图像数据尺寸的动态调整、视频动态抽帧等操作。
- [vit](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L404)：对视觉数据进行处理。在输入部分，使用3D conv把视觉数据做成14*14 patch分块，并使用window attention节省计算。在输出部分，通过一个mlp层将2*2个patch块合并成一个merged_patch，这个merged_patch代表的token才是qwen2.5 LM decoder中的一个输入。
- [Qwen2.5 LM Decoder](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L1102)：对文本和视觉数据使用3D M-rope（3D多模态rope），然后送入主体模型中。
...
**代码在[这里](https://github.com/QwenLM/Qwen2.5-VL/blob/c15045f8829fee29d4b3996e068775fe6a5855db/qwen-vl-utils/src/qwen_vl_utils/vision_process.py#L97)** 
...
（2）通过四舍五入的方式，重新设置图片的 h 和 w 值，确保它们可以被28整除
（3）如果这张图片太大，超过了上述 max_pixels 的范围，那么就在**尽量维持其宽高比例不变的情况下，重新计算其符合max_pixels范围的h和w。图片太小也是同理。**  （4）经过前面的步骤，我们得到了这张图片最终理想的h和w值（resized_height，resized weight），我们采用
...
- 入口代码在[这里](https://github.com/QwenLM/Qwen2.5-VL/blob/c15045f8829fee29d4b3996e068775fe6a5855db/qwen-vl-utils/src/qwen_vl_utils/vision_process.py#L277)
- 核心处理代码在[这里](https://github.com/QwenLM/Qwen2.5-VL/blob/c15045f8829fee29d4b3996e068775fe6a5855db/qwen-vl-utils/src/qwen_vl_utils/vision_process.py#L226)。（假设用的处理后端是decord，当然还有torchvision之类的可选，处理逻辑都差不多）
...
下面对代码中`smart_nframes`这个对原始视频帧的采样过程进行解读。
...
**最后，再来看来自这份代码里的、关于video部分的一个重要[返回args](https://github.com/QwenLM/Qwen2.5-VL/blob/c15045f8829fee29d4b3996e068775fe6a5855db/qwen-vl-utils/src/qwen_vl_utils/vision_process.py#L375)** :
...
![](./assets/v2-257917c4c3ba173a6f7bf48439108ae2_r.jpg)
...
- 我们对text数据做了add_chat_template的处理，增加了一些表示start，end、image、video的placeholder（参见一）
...
- Processor入口函数在[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/processing_qwen2_5_vl.py#L48)，其中[这个部分](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/processing_qwen2_5_vl.py#L129-L142)计算了3.2中所说的`second_per_grid_ts`，这个值将在mrope中被使用。
- Procssor中，负责处理vision数据的`self.image_processor`核心实现在[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_vl/image_processing_qwen2_vl.py#L87)。
- Processor中，负责处理text数据的`self.tokenizer`比较常规，这里不展示细节。

### 4.1 self.image_processor: Qwen2VLImageProcessor

- 代码在[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_vl/image_processing_qwen2_vl.py#L87)
...
![](./assets/v2-ac50b6222eab18e66df2b0984f2e1a0b_r.jpg)
...
tokenizer的具体内容不展开，这里只强调，对于原始text部分，在进入tokenizer前我们还要做一些处理，处理核心代码在[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/processing_qwen2_5_vl.py#L151-L177)。
...
- **每一张图对应的token数量为：(grid_t * grid_h * grid_w)/ (merge_size**2)，其中merge_size表示你想让多少个patch合并成一个token，默认值是2，即在原始图像上做出14*14大小的patch后，我将2*2个patch合并成1个token表示。根据这个计算方式我们扩展`<|image_pad|>`。** 
- 按这个方式处理完text后就可以送入self.tokenizer取做tokenize了，会按照max_len做截断，也会在一个批次中做padding
...
处理好了text和vision，现在就可以送入模型开始正式做fwd了。整个qwenVL模型架构在`Qwen2_5_VLForConditionalGeneration`中，其中包含了VIT encoder和qwenVL主体decoder。这里我们只关注VIT encoder（`Qwen2_5_VisionTransformerPretrainedModel`），下面介绍vit encoder的forward方法。

### 5.1 使用3D Conv将原始图像patch转变为vit的输入token

- 这个过程也称为embedding
- 这部分的代码在[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L88)。

在进入vit encoder前，我们vision部分的输入如下，我们会有`flatten_patches + 对应的(grid_t, grid_h, grid_w)`数据作为输入。

![](./assets/v2-ac50b6222eab18e66df2b0984f2e1a0b_r.jpg)

**那么3D conv要做的事情就是（相当于拿out_channels（= vit_hidden_size）个3D kernel，每个kernel都在所有patch方块上滚一遍， 得到hidden_size的一个数值，所有out_channels个kernel滚完就得到了完整的hidden_size）：** 

![](./assets/v2-64b49e351a9db01e9f209d4d63172710_r.jpg)
...
![](./assets/v2-da4233aabcd42f7a1e0d5456a349f71c_r.jpg)

*截图的我自己的rope文章*

**注意，Rm矩阵的解不是唯一的，例如1D rope的Rm还可以写成下面这种形式，因为它同样满足下面蓝体公式（这也是llama系列的改写方式），在qwenvl后续的各种rope中，沿袭了此种方法（代码更好写了）：** 

![](./assets/v2-8d0e374c164c282e3ef5d3d56dc7bbd8_r.jpg)

*请给我一点时间找下这张图片链接，应该是来自huggingface的某个discussion*

**那么对应到vit 2D rope的某个token中，它的2D位置编码如下（这里vit_hidden_size指的是head_dim）：** 

![](./assets/v2-1514637451e1bff4e7af5ff1aa128232_r.jpg)

*是我的丑字，希望没有辣到大家眼睛*

**虽然看起来麻烦，但可以粗糙理解成半个head_dim在负责做h位置的rope，半个head_dim在负责做w位置的rope，这两个部分共享一套**  $\theta$ **值** 
详细实现可以去看代码，分别在[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L114)和[这里](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L436)
...
![](./assets/v2-285daabd1a4a0e9563997736a416abad_r.jpg)

*根据qwen代码原创绘制*

但是按照5.1节中的展开方法，现在我们输入vit模型的tokens排序如下，这种方式下同一个窗口内的patch（也就是vit tokens）无法连续地排列在一起！

![](./assets/v2-7ca3bbda05d26d5beef75f9e6a114c05_r.jpg)

*根据qwen代码原创绘制*

**为了做window attention，我们希望：** 

![](./assets/v2-a635f83d96f40ad6df357485cf26f9ba_r.jpg)

*根据qwen代码原创绘制*

这种调换vit tokens顺序的方法，就是[这块代码](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L519-L535)在做的事情。我们按照这个顺序作为vit的输入，然后正常经过vit blocks，得到最后的输出，但是要注意，在输出结果中，我们必须要把顺序再次调换回来。
...
在整个vit的处理中，我们都是使用14*14的patch作为一个token处理的，但是最终进入到qwenvl decoder（也就是主体模型）中时，我们需要将原来2*2个patch合并成一个token，这可以通过先把2*2个token的结果concat起来，然后经过一个mlp层做hidden_size维度的映射得到，如下图：

![](./assets/v2-1f708b3a85f0cd60557f2e7477270cd5_r.jpg)

*根据qwen代码原创绘制*

这就是[这块代码](https://github.com/huggingface/transformers/blob/41925e42135257361b7f02aa20e3bbdab3f7b923/src/transformers/models/qwen2_5_vl/modeling_qwen2_5_vl.py#L146)在做的事情。
...
![](./assets/v2-e6d08c686a68fb25fb3be108a9c6c4e1_r.jpg)
...
![](./assets/v2-6541a3263f76f32a2987e9d399ec803d_r.jpg)
...
![](./assets/v2-086d610a300f59b3257bf1ff71829359_r.jpg)
...
好，基于这些理解，则现在`second_per_grid_t * tokens_per_second`这个乘积，就表示基于视频帧的绝对时间间隔（单位s），去修复它在T方向上理论上每秒应该走过的tokens数，所以这个乘积最终就是T维度上的position_id，用来替换原来的默认值1。举个例子：
...
![](./assets/v2-b7658c166a07d63af591ab38875a493a_r.jpg)
