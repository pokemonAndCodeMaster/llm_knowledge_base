...
- **参数量**：$3 \times 1152 \times 2 \times 14 \times 14 = 1,354,752 ≈ 1.35M$ 参数。
- **参数来源**：继承自大规模预训练的 Vision Transformer。
- **训练状态**：
  - Stage 1（ViT + PatchMerger 训练）：**可训练**，随 ViT 一起训练。
  - Stage 2/3（全模型训练）：**可训练**，全量解冻微调。
  - 后训练 SFT/DPO 阶段：**冻结**（ViT 整体被冻结）。
