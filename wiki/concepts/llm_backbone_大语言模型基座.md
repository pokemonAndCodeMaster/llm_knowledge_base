...
### 5. 多模态与提效前沿：DeepStack 与 MTP
*   **DeepStack 跨层特征融合**：
    在 Qwen3-VL 中，不仅将 ViT 最后一层的特征送入 LLM，还将 ViT 中间层（如第 8, 16, 24 层）的特征提取出来，**直接跨层注入到 LLM 的前 K 层中**（通过残差相加的方式仅作用于视觉位置的隐状态）。这种强行将低级视觉细节（如边缘、颜色）和高级语义结合的方式，大幅度增强了细粒度理解。
*   **MTP (Multi-Token Prediction)**：
    在 Qwen3.5 (如 397B 模型) 的最顶层（第 61 层）额外增加了一个“草稿模型层”。在一次前向传播中，模型不仅预测下一个 Token，还顺带预测下下个 Token。这种技术在 DeepSeek-V3 中也大放异彩，能在保证精度的前提下，让解码吞吐量提升 1.5 倍左右。
