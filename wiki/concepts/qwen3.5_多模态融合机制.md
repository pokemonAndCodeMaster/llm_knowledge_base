...
Qwen3_5Model.forward(input_ids, pixel_values, image_grid_thw, mm_token_type_ids, ...)
  ...
  │     → image_embeds: [tensor(256, 4096)]  按图片拆分
  │     → cat → (256, 4096)
  │
  ├─ 3. image_mask, _ = get_placeholder_mask(input_ids, inputs_embeds, image_embeds)
  │     找到 input_ids 中 == 248056 的 256 个位置
  │     image_mask: (1, 268, 4096) bool
  │
...
def get_placeholder_mask(self, input_ids, inputs_embeds, image_features=None, video_features=None):
    # input_ids: (1, 268), 其中位置 5-260 是 image_token_id=248056
    special_image_mask = (input_ids == 248056)
    # shape: (1, 268), dtype: bool, 有 256 个 True

    n_image_tokens = special_image_mask.sum()  # = 256

    # 扩展为 3D: (1, 268) → unsqueeze(-1) → (1, 268, 1) → expand → (1, 268, 4096)
...
**推演**：假设 input_ids = `[BOS, <vision_start>, <image>×256, <vision_end>, 描, 述, 这, 张, 图, 片, ...]`
- 位置 2-257（256 个 `<image>`）的 embedding 原本是随机的占位值
- `masked_scatter` 后这 256 个位置被替换为 ViT 编码后的视觉特征向量
- 其他位置（文本 token）的 embedding 不变
...
def compute_3d_position_ids(self, input_ids, inputs_embeds, image_grid_thw, ...):
    ...
    return position_ids  # (3, 1, 268)
    # → 拼上 text 维后变成 (4, 1, 268)
```
...
假设输入序列（简化）：`[BOS(文本), <vs>(文本), <img>×256(图像), <ve>(文本), 描(文本), 述(文本)]`

mm_token_type_ids = `[0, 0, 1×256, 0, 0, 0]`

分组：
1. 文本段 (pos 0-1): `current_pos=0`, T=H=W=`[0,1]`, `current_pos→2`
2. 图像段 (256 tokens): grid_thw=[1,32,32], merge=2 → 16×16=256 tokens
   - T 维: 全部 = 2（current_pos=2, T=1帧）
   - H 维: `[2,2,...,2, 3,3,...,3, ..., 17,17,...,17]`（16行，每行16列）
   - W 维: `[2,3,...,17, 2,3,...,17, ..., 2,3,...,17]`（16列，重复16行）
   - `current_pos += 16` → `current_pos=18`
3. 文本段 (pos 258-262): T=H=W=`[18,19,20,21,22]`

最终 position_ids shape: `(3, 1, 268)`
