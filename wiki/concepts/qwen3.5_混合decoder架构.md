...
沿用 [[qwen3.5_前向传播全链路]] 的统一样本：seq_len=921, hidden_size=4096。
...
Qwen3_5TextModel.forward(inputs_embeds, position_ids, attention_mask)
  ...
  输入: inputs_embeds (1, 921, 4096), position_ids (4, 1, 921)
  │
  ├─ text_position_ids = position_ids[0]   # (1, 921) — 给 causal mask
  │  position_ids = position_ids[1:]        # (3, 1, 921) — 给 MRoPE [T,H,W]
  │
  ├─ causal_mask = create_causal_mask(...)  # (1, 1, 921, 921) 下三角
  │  linear_attn_mask = attention_mask      # (1, 921) 仅 padding mask
  │
  ├─ position_embeddings = self.rotary_emb(hidden_states, position_ids)
  │  # Interleaved MRoPE → (cos, sin) 各 (1, 921, 128)
...
| Mask 类型 | 适用层 | 形状 | 说明 |
|-----------|--------|------|------|
| `causal_mask` | Full Attention | `(1,1,921,921)` | 标准下三角因果 mask |
| `linear_attn_mask` | GatedDeltaNet | `(1,921)` | 仅 padding mask（无因果性） |
...
class Qwen3VLTextAttention(nn.Module):
    ...
    def forward(self, hidden_states, position_embeddings, attention_mask, past_key_values, **kwargs):
        # hidden_states: (1, 921, 4096)
        input_shape = hidden_states.shape[:-1]  # (1, 921)

        # 1. 投影 + QKNorm
        query = self.q_norm(self.q_proj(hidden_states).view(1, 921, 32, 128)).transpose(1, 2)
        # q_proj: (1,921,4096) → (1,921,4096) → view → (1,921,32,128)
        # q_norm: per-head RMSNorm on dim=128 → (1,921,32,128)
        # transpose → (1, 32, 921, 128)

        key = self.k_norm(self.k_proj(hidden_states).view(1, 921, 4, 128)).transpose(1, 2)
        # → (1, 4, 921, 128)

        value = self.v_proj(hidden_states).view(1, 921, 4, 128).transpose(1, 2)
        # → (1, 4, 921, 128)  无 norm

        # 2. 应用 RoPE
        cos, sin = position_embeddings  # 各 (1, 921, 128)
        ...
        # attention_interface 内部: repeat_kv(key, 8) → (1,32,921,128)
        # attn = softmax(Q @ K^T / sqrt(128) + mask) @ V
        # output: (1, 32, 921, 128) → transpose → (1, 921, 32, 128)

        # 5. 输出投影
        attn_output = attn_output.reshape(1, 921, 4096)
        attn_output = self.o_proj(attn_output)  # → (1, 921, 4096)
...
class Qwen3_5MLP(Qwen3NextMLP):
    ...
    def forward(self, x):  # x: (1, 921, 4096)
        return self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
        # gate: (1,921,12288), up: (1,921,12288)
        # SiLU(gate) × up → (1,921,12288)
        # down → (1,921,4096)
```
