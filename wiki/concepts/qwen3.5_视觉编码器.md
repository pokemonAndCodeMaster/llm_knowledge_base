...
沿用 [[qwen3.5_前向传播全链路]] 的统一样本：1 张 448×448 图。
...
Qwen3_5VisionModel.forward(pixel_values, grid_thw)
  ...
  输入: pixel_values (1024, 3, 2, 14, 14), grid_thw (1,3)=[1,32,32]
  │
  ├─ 1. patch_embed(pixel_values)
  │     Conv3d(3, 1280, kernel=(2,14,14), stride=(2,14,14))
  │     输入: (1024, 3, 2, 14, 14) → reshape(-1, 3, 2, 14, 14)
  │     输出: (1024, 1280)
  │
  ├─ 2. fast_pos_embed_interpolate(grid_thw)
  │     双线性插值绝对位置嵌入
  │     输出: pos_embeds (1024, 1280)
  │     hidden_states = hidden_states + pos_embeds  → (1024, 1280)
  │
  ├─ 3. rot_pos_emb(grid_thw) → 2D RoPE
  │     输出: rotary_pos_emb (1024, 40)
  │     → emb = cat(rotary, rotary) → (1024, 80)
  │     → position_embeddings = (emb.cos(), emb.sin())
  │
  ├─ 4. cu_seqlens 计算
  │     = [0, 1024]  # 1张图作为1个attention段
  │
  ...
  └─ 6. merger(hidden_states)  → PatchMerger
        2×2合并: 4个相邻token拼接
        Norm → Linear(5120,5120) → GELU → Linear(5120,3584)
        输出: (256, 3584)
```
...
#### 逻辑链输入与输出
- **输入**：`pixel_values` (1024, 3, 2, 14, 14) — 1024 个 patch group
- **输出**：`hidden_states` (1024, 1280) — 每个 patch group 的特征向量
...
**推演**：448×448 图，T=1（图像），但 temporal_patch_size=2 需要 2 帧 → 图像被复制为 2 帧。H=448/14=32, W=448/14=32。每个 patch group 覆盖 2×14×14 像素，共 32×32=1024 个 group。Conv3d 将每个 (3,2,14,14)=1176 个像素值投影到 1280 维。
...
class Qwen3VLVisionPatchMerger(nn.Module):
    def __init__(self, config):
        ...
        self.act_fn = nn.GELU()
        self.linear_fc2 = nn.Linear(5120, 3584)  # out_hidden_size

    def forward(self, x):
        # x: (1024, 1280)
        x = self.norm(x)  # 对1280维做LayerNorm
        x = x.view(-1, 5120)  # 每4个相邻token拼接: (256, 5120)
        x = self.linear_fc2(self.act_fn(self.linear_fc1(x)))
        # (256, 5120) → (256, 5120) → (256, 3584)
        return x
```

**推演**：1024 tokens ÷ 4 = 256 merged tokens，每个 merged token 由 4×1280=5120 维 → FC1 → GELU → FC2 → 3584 维。
