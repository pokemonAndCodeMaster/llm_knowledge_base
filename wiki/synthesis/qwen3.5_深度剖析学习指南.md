...
---
## 第 4 章：原生多模态训练范式 (Native Multimodal Training)

### 1. 架构顺序解读
**来龙去脉**：Qwen3.5 之所以被称为“原生多模态”，并非视觉端架构变了，而是其**底层训练工程与基础设施**发生了质变。它从拼接式的分阶段解冻，走向了真正的图文混合、异构并行、多 Token 预测的端到端原生范式。

### 2. 概念学习序列 (黄金准出标准)
*   **【概念一：异构基础设施与解耦并行】**
    *   **黄金准出条件**：明白前端 ViT 是吃显存的 Dense 网络，后端 LLM 是吃通信带宽的稀疏 MoE 网络。解释 `ms-swift` 和底层训练框架是如何让这两种算子在 GPU 集群上各跑各的，通过异步传送隐状态来消除木桶效应。
    *   👉 **深度学习去这里**：**[[qwen3.5_原生多模态训练范式#架构解耦：异构基础设施-heterogeneous-infrastructure]]**

*   **【概念二：多阶段原生联合训练 (Joint Pre-training)】**
    *   **黄金准出条件**：对比 2.5-VL，能说出 3.5 是从极早期（Stage 1）就开始图文混炼的，而不是先训好文本大脑再学看图。了解 Stage 1 到 Stage 3 在序列长度和数据规模上的扩展。
    *   👉 **深度学习去这里**：**[[qwen3.5_原生多模态训练范式#核心训练阶段拆解与数据配比]]**

*   **【概念三：MTP 联合加速提效】**
    *   **黄金准出条件**：不仅能在推理时解释 MTP 草稿层，还能解释为什么在预训练时加入 MTP（预测下一个和下下个 Token），能**倒逼视觉编码器提取出更具长远逻辑连贯性的物理特征**。
    *   👉 **深度学习去这里**：**[[qwen3.5_原生多模态训练范式#mtp-multi-token-prediction-联合提速训练]]**
